...
**读报告前必须知道的一件事**（详规 §11.2 的实算）：

    配对 + Wilson 单侧 95% 上界下，70 条任务只容 **3 条**"B1 过、B2 不过"的
    翻转；**10 条试水容差为 0**。所以本报告只用于：打通链路、看方向、
    验证档位开关真的在改变行为。**它不能下结论**——结论要等 70 条。
...
#: B1 → B2 方向的翻转（"B1 过了、B2 反而没过"）——5% 门量的就是这个方向。
...
def main() -> int:
    ...
    md.append(
        ...
        + (
            ...
            "实测坐实）。70 条正式集必须改：描述降噪（不给症状细节，让 agent 自己"
    ...
    md.append("- [ ] 正式结论：**待 70 条任务集**（P2 门第 3 条），且任务难度要先改（见发现 1）")
    md.append("")
