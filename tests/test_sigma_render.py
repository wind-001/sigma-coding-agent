...
from sigma_agent.observe import TextChunk, ToolEnd, ToolStart, TurnEnd
...
def test_failed_tool_is_marked_with_cross_and_text_is_shown() -> None:
    ...
    """
    out = _render(ToolEnd(name="bash", ok=False, preview="命令失败，退出码 1。"))
    assert "✗" in out
    assert "bash" in out
    assert "退出码 1" in out

    ok_out = _render(ToolEnd(name="bash", ok=True, preview="total 3"))
    assert "✓" in ok_out
