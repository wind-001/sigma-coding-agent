...
> 当前状态（截至提交 `4af620e`）：**P1 批次 0 / 1 / 1.5 已完成；
> 批次 2–4 的 agent loop 已跑通**（`sigma -p "任务"` 可用）。
> 当前盘上 `185 个单测全绿`、三道门禁全绿、注入实验 **17/17** 证伪成功
...
`.env` 支持：`KEY=value`、`# 注释`、空行、可选的 `export` 前缀、值两边的引号。
**不支持变量展开**——值里的 `$` 会按字面处理（并且会**告警**，不静默带过，
因为"以为展开了其实没有"会让 key 变成一个诡异字符串，而症状是 401，不指向根因）。
