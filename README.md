...
> 当前状态（截至提交 `0259e42`）：**P1 批次 0、批次 1、批次 1.5 已完成**；
> 批次 2–4 的计划已写好待审（`docs/plans/P1-批次2-4-详规.md`）。
> 当前盘上 `151 个单测全绿`、三道门禁全绿、两个注入实验 **17/17** 证伪成功
> （批次 1 的 7 条 + 批次 1.5 的 10 条）。
>
> **Provider 层已于 2026-09-20 对 DeepSeek 真实端点冒烟通过（5/5）**——
> 但覆盖范围只有 `sigma_ai` 一层，**agent loop 尚未实现**。
> 未验证项见 `docs/plans/P1-批次1-详规.md` 第 9 节（§9.1 是真实 API 复核结果，
> §9.2 记了一处测试设计缺陷）与 `P1-批次1.5-详规.md` 第 9 节。
>
