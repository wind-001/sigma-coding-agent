...
> ——**467 单测全绿 · 三道门禁全绿（mypy strict 51 files / 契约 3 kept）· 73 条门槛注入全部可证伪 · 回放 6/6**。
...
类型 → `mypy --strict`；行为 → 467 个单测。**CI 就这三条命令，跑不过不合。**
...
tests/          467 个单测（不需要 API key）；fixtures/transcripts/ 是六个回放场景
