...
>
> **v1.5 变更**（2026-09-21，批次 8）：
> - **新增可选内置工具 `web_fetch`**（网页精读，Firecrawl）：与 `web_search` 同为"有 key 才注册"的可选位。
>   **单次最多精读 2 条 URL**（框架硬闸，截断发生在扣额度之前）；一次 scrape = 1 credit，
>   免费额度 1000 credits/月（每月刷新），账本见 `core/sigma_tools/_firecrawl_quota.py`。
> - **`web_search` 升级为"会调研"**：结果进入上下文之前先过两道硬规则
>   （低质量来源黑名单 + 时间预过滤，见 `core/sigma_tools/_source_policy.py`），
>   丢弃条数写回给模型看的内容里；每条结果带可识别的发布时间。
> - **系统提示词新增"调研纪律"段**（约 +300 token）：何时精读、引用要带 URL 与时间、
>   新旧冲突采信更新者、只有旧资料要直说。
...
| **可选工具 `web_search` + `web_fetch`**（2026-09-21 新增，批次 7/8） | 联网搜索（Tavily）+ 网页精读（Firecrawl）：查本地工作区里没有的信息（库的最新用法 / 报错原因 / 版本变更 / 事实核对），摘要不够时精读单页正文。**不属于核心 5 工具**——工具 schema 进常驻区，所以只在配了对应 key 时注册，`--no-web-search` 可整体关。免费额度各 1000 credits/月，超额自动禁用；`web_fetch` 单次最多 2 条 URL（框架硬闸） |
...
这不是措辞问题：按后者，摘要是**动态区**的内容（降级成 `user` 消息追加到尾部），
与"逐字节稳定"无关；按旧表的读法，它会白占 1,000 token 常驻预算。
**P2-3 的口径冲突排查（下面 Q5 那一段）就是被这张表带偏的。**

| 分区 | 预算（token） | 加载方式 | 可变性 |
| --- | --- | --- | --- |
| 系统提示词 | ≤ 400 | 启动时固定 | 会话内不可变 |
| 工具 schema（6 个核心，含 `load_skill`） | ≤ 1,300 | 启动时固定 | 会话内不可变 |
| 工具 schema（2 个可选） | +≤ 750 | 启动时按 flag | 会话内不可变 |
| `AGENTS.md` | ≤ 700 | 启动时注入，硬截断 | 会话内不可变 |
| 技能索引（name + description + location） | ≤ 350 | 启动时固定 | 会话内不可变 |
| **常驻小计** | **≤ 3,500** | | **逐字节稳定** |
| checkpoint / 摘要 | 不计入常驻区 | 压缩产出，降级成 `user` 追加到尾部 | 随压缩更新 |
| 会话历史 + 工具输出 | 剩余 | 动态 | append only |

#### 5.1.0 ⚠️ 上表的各分项上限此前**从来没在 3,500 里装下过**

**这是 2026-09-22（P4-批次1）才发现的问题。** 上一版的五条上限相加是
`800 + 1,100 + 750 + 800 + 600 = 4,050` —— **比常驻小计多了 550**。

也就是说，**这张表在纸面上就是自相矛盾的**：它一边说"常驻小计 ≤3,500"，
一边把每个分区的上限都写到总和 4,050。而因为**没有任何东西在检查这件事**
（G59 检查的是实测值，不是这些 cap），它一直没被发现。

**处置：重新分配，使 cap 之和 = 3,500。** 各 cap 不再拍脑袋，而是
"实测值 × 留出可解释的余量"：

| 分区 | 实测（2026-09-22） | 新 cap | 余量 | 余量留给什么 |
| --- | --- | --- | --- | --- |
| 系统提示词 | 282 | **400** | +42% | 提示词还要写"工作方式"与调研纪律 |
| 工具 schema（核心 6） | 1,220 | **1,300** | +6.5% | 工具描述再长一点就到顶——**这条最紧** |
| 工具 schema（2 可选） | 711 | **750** | +5.5% | 同上 |
| `AGENTS.md` | 104 | **700** | 充裕 | 它是用户写的文件，不该被我们勒住 |
| 技能索引 | 57 | **350** | ≈15 个技能 | 每条约 22 token；超过 15 个该做二级索引 |
| **合计** | **2,374** | **3,500** | 实测余 **1,126** | |

**教训（值得单独记）**：*"小计 ≤ N"与"分项各 ≤ nᵢ"必须同时成立*，
而后者要**真的加起来验一遍**。只写一个总数、再给分项各写一个"看着合理"的上限，
得到的就是一张**看起来严谨、实际自相矛盾**的表。
这和「配置跑绿 ≠ 约束生效」是同一族：**没被检查过的约束，等于没有约束**。

#### 5.1.1 实测（2026-09-21，口径 = `sigma_ai.tokens.estimate_text` 粗估）

| 组成 | 表里的值 | **实测** |
| --- | --- | --- |
| 系统提示词（核心五工具） | ≤ 800 | **131** |
| 系统提示词（+两席联网） | — | **286** |
| 工具 schema（5 个核心） | ~820 | **1,018** |
| 工具 schema（+两席） | +~450 | **1,729**（即 +711）|
| `AGENTS.md` 上限 | 1,500 | **800**（见下）|

**实测与旧表的估算有出入**：schema 被低估约 200，可选栏被低估约 260。
旧表的三处数字是**估算**，不是测量——现在补上测量值，估算值作废。

#### 5.1.2 为什么 `AGENTS.md` 的上限从 1,500 收到 800

按 D4 的定义算一遍（两个联网工具全开）：

```
286（提示词）+ 1,729（schema）+ 1,500（AGENTS.md）= 3,515  >  3,500   ✗ 超 15
286 + 1,729 +   800                              = 2,815  ≤  3,500   ✓ 余 685
```

**两个数字都要说清楚，因为它们对应两个不同的判断**：

1. **现在的超限量是 15 token，不是 1,607。** P2 详规 Q5 当时算出的
   "≈5,107 / 超 1,600+" 把**摘要**（1,000，按 D4 定义不在常驻区）
   与**技能索引**（600，P4 才做）都加了进去——**重复计算**。
   这是我的错，记在这里以免后人照抄那份结论。
2. **但收紧到 800 仍然是对的**，理由换成了"给 P4 留空间"：
   `2,815 + 600（技能索引）= 3,415 ≤ 3,500` ✓。
   若维持 1,500，则 P4 一加技能索引就是 `3,515 + 600 = 4,115` ✗。

   **所以这条改动的理由是"预算要留给已知的下一项"，不是"现在快爆了"。**
   把理由说准很重要——说成后者会让下一个人以为 D4 已经很紧张，
   于是倾向于下一次继续放宽。

**不放宽 D4**：它是 prompt cache 的经济性主张。为让实现通过而放宽它，
等于把主张改掉以迁就实现。

#### 5.1.3 这道预算现在有代码硬闸了（G59）

`SessionContext.verify_resident_budget()` 在每次 `build_messages()` 前校验，
超限抛 `ResidentBudgetExceeded`（`sigma_session/context.py`）。
理由：常驻区**每轮都要重付一次**，而超限的症状是"变慢变贵"——
静默、且离根因很远。**把它从表里的数字变成会抛的断言。**
...
>
> **批次 8 追加**：第二个可选工具 `web_fetch`（精读）加入同一栏，
> 系统提示词新增"调研纪律"段。两席合计实测 **+711 token**。
> 再往可选栏加东西之前，先跑 7.6 的 ablation。
>
> **P2-3 追加**：`AGENTS.md` 已接入常驻区，上限 800 token、硬截断且
> **截断在正文里可见**（`sigma_session/resources.py`）。
> 常驻区现在有指纹（G26/G50）与预算（G59）两道断言。
...
**P2-3 追加：还有第二道断言——预算。** 常驻区**超过 3,500 也要抛**
（`ResidentBudgetExceeded`，门槛 G59）。理由是它与"变化"一样是静默失败：
超限不会报错，只会让每一次请求都更贵。两道断言的分工是
**指纹管"变没变"，预算管"大不大"**——它们不是同一件事的两半，
而是两个独立的失败模式，所以各有一条门槛、各有一个能证伪它的注入实验。
...
**为什么补这一节**：上面那段排期只有"约 N 周"的粒度，而
**文档此前只记日期、不记时刻**——于是"P1 什么时候完成的"这类问题
无法回答，排期与实际也就无从对照。从 P2-1 起改为**每个阶段都记到分钟**。

**数据来源要说清楚**（哪些是实测、哪些是推算）：

| 标记 | 含义 |
| --- | --- |
| **实测** | 取自 `git log` 的提交时刻，**分钟级精确** |
| 推算 | 由相邻提交时刻作差得到（含睡眠与休息，**不等于工时**）|
| 估计 | 由会话起止推得，**是墙钟时长而非专注工时**，只能作量级参考 |

| 时刻（2026，本地） | 提交 | 事件 | 段内耗时 |
| --- | --- | --- | --- |
| 09-20 15:21 | `81d2f21` | **P0 骨架** | — |
| 09-20 16:28 | `48aba26` | 架构方案定稿 v1.2（按 Pi 源码重定消息模型）| P0 段 **1h07m** |
| 09-20 17:55 | `ffadd99` | P1 批次 1（sigma_ai，注入 7/7）| 1h27m |
| 09-20 20:08 | `0b6895a` | P1 批次 1.5（agent 层消息；顺带移除 ruff）| 2h13m |
| 09-20 22:50 | `110f252` | P1 批次 2–4（工具机制 + loop，167 测试）| 2h42m |
| **09-21 00:47** | `68521c6` | P1 批次 2–4 收尾（三工具落地）| 1h57m |
| 09-21 09:05 | `303f344` | P1 批次 6（流式渲染 + 简版 REPL）| — |
| 09-21 14:01 | `e52888f` | P1 批次 7（`web_search`）| 4h56m |
| **09-21 23:55** | `615b896` | P1 批次 8 + **P2-1 / P2-2 / P2-3** | 9h54m |
| **09-22 09:03** | `b0590e3` | **P2-4**（上下文压缩）| — |

**阶段跨度（推算）**

| 阶段 | 起 | 止 | 日历跨度 |
| --- | --- | --- | --- |
| P0 | 09-20 15:21 | 09-20 16:28 | **1h07m** |
| P1 | 09-20 16:28 | 09-21 23:55 | **31h27m** |
| P2（至 P2-4）| 09-21 20:04（P2 详规落盘，估计）| 09-22 09:03 | **约 13h** |

**与排期的对照**

排期估的是"**P1 与 P2 各约 3 周 @ 8 小时/周**" = 两个阶段共 **48 小时**。
实际投入（按会话墙钟，**估计**）：

| 会话 | 区间 | 时长 |
| --- | --- | --- |
| ① | 09-20 15:21 → 09-21 00:47 | 9h26m |
| ② | 09-21 09:05 → 14:01 | 4h56m |
| ③ | 09-21 约 19:00 → 23:55 | 约 4h55m |
| ④ | 09-22 00:02 → 00:49 | 47m |
| ⑤ | 09-22 08:42 → 09:03+ | 21m+ |
| **合计** | | **约 20h25m** |

**约 20.4 小时 / 计划的 48 小时 ≈ 42%**——比附录里"若 sigma 成为唯一主载体
可压缩约 30%"的估计**还要快**。三个可能的原因（不互相排斥，且都未被验证）：

1. 排期按"学期中 8h/周"的**碎片时间**估，而实际是**连续大块**（单次最长达 9h26m）——
   上下文切换成本低得多；
2. 估工时的对象是"含评测与 ablation 的完整 P1–P4"，而 P1 批次 5
   （70 条任务集）与 P5 尚未做——**分母与分子口径不同**，42% 不能当"效率"看；
3. 门禁与注入实验把返工**前置**了，所以看起来快是因为坑在早期就踩完了。

**⚠️ 因此这一节只用来对照，不用来宣称效率。** 真正可比的数字要等
`evals/datasets/` 落地、P2 第 3 条验收门跑完之后才有。
...
| 内置工具 | 4（+3 可选） | **5 + 2 可选**（`web_search` / `web_fetch`） | 比 Pi 多 `grep`：4 个覆盖不了跨文件搜索定位；联网工具是可选的第 6、7 个（有 key 才注册），Pi 的同类能力走 MCP 扩展 |
