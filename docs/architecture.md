...
| 4 个内置工具 read / write / edit / bash | 构成最小闭环；`edit` 必须是**精确字符串替换 + 输出 unified diff**，不是整文件重写 |
...
| 工具 schema（4 个） | ~700 | 启动时固定 | 会话内不可变 |
...
| 工具 7 → 4（去掉 grep/find/ls） | 7 工具版本 | 成功率、工具调用轮数 |
