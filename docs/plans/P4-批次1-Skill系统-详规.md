...
6. 门槛 G75–G79 + 注入实验（`scripts/gate_injection_batch14.py`）
...
| `tests/test_agent_skills.py`（新） | **43 个用例** |
| `scripts/gate_injection_batch14.py`（新） | G75–G79（5/5 可证伪） |

### 12.2 实测数字（绑本次提交）

- `pytest -q` → **510 passed**（467 → 510，新增 43）；
- `mypy core` → **53 files clean**；
- `lint-imports` → **3 kept, 0 broken**（**兄弟层契约没被破坏** → `skills.py` 放 `sigma_agent` 是对的）；
- 注入实验 → **78/78**；另 `gate_injection_p2_compaction.py` **2/2**；
- 回放场景 → **6/6**；无注入残渣。

> ⚠️ **本节数字改过一次，而原因值得单独记下来。**
>
> 第一版写的是「498 / **81**」（对应提交 `3310dcb`），其中 **81 是错的**：
> §12.5 里列的明细 `7+10+10+6+5+7+4+3+4+4+4+9+4` **自己加起来是 77**。
> 一个数字和它自己列出的明细对不上，**说明当时谁都没有做过那次加法**。
>
> 现在这份 **78** 是**逐脚本实跑数出来的**（batch14 由 4 条增至 5 条，见 §12.6）。
>
> 这正是「计数绑提交号、换代码必须重跑、不能照抄」那条纪律的实例——
> 只不过这次腐化的是**文档里的数字**，不是代码里的。
> 而且它比代码腐化更难发现：**没有任何门禁会检查散文里的数字**。
...
本批改了 `context.py` / `sdk.py` / `cli.py` / `resources.py` / `skills.py` / `skill.py`，
按元纪律②重跑 **batch1–batch14**（**逐脚本实跑，不是照抄**）：

**7+10+10+6+5+7+4+3+4+4+4+9+5 = 78/78**，另 `gate_injection_p2_compaction.py` 2/2。

（§12.5 第一版这里写的"= 81/81"是错的，那条明细自己加起来只有 77。更正说明见 §12.2 的注。）

---

## 13. 补丁：G79 —— 截断说明不能被预算吃掉

**状态：✅ 已修。** 落盘 2026-09-23，这是提交 `3310dcb` **之后**的补丁。

### 13.1 缺陷是怎么暴露的

`3310dcb` 提交后，我在**提交后的树上**重跑全量：

```text
单文件跑  tests/test_agent_skills.py      → 31 passed   ✅
全量跑    python -m pytest -q              → 1 failed    ❌
```

失败的是 `test_load_body_truncates_with_visible_marker`。**单跑绿、全量红**，
第一反应是"测试顺序依赖"——但**不是**。

### 13.2 根因：标记里带了随环境变化的量

`load_body` 的**详细标记**里含 `skill.source`（**绝对路径**），而路径长度随环境变：

| 环境 | 临时根 | 详细标记长度 |
| --- | --- | --- |
| 系统 Temp（`AppData\Local\Temp`） | 约 90 字符 | 约 145 字符 |
| **指进项目**（`.pytest_cache/tmp`） | 130+ 字符 | 约 190 字符 |

于是同一个 `max_tokens=200`：一种环境切得下，另一种环境下**标记自己就把预算吃光**
→ `truncate_to_tokens` 的候选标记全被否 → 返回**空文本**。

**危害不是"截断得更多"，是"丢弃变得不可见"**——
调用方拿到一段既没正文、也没说明的结果。而"丢弃必须可见"是 D4 那条纪律的地基。

### 13.3 这是同一个坑第二次踩到

`resources.py`（AGENTS.md 的注入）**修过一模一样的坑**，当时的处置就是加短标记退路。
技能这边漏了。**同一个坑能活到全量跑，说明当时没有一条与环境无关的用例钉它。**

### 13.4 修法（两处）

1. **`sigma_agent/skills.py`**：`load_body` 加 `short_marker` 退路
   ——它只用 `location`（**相对**路径），长度与环境无关。
2. **`sigma_tools/skill.py`**：空文本的报错**分两种**。原先三种原因合成一句
   "正文读不出来（权限 / 编码 / 文件被删）"，于是"预算装不下"会让人**去查文件系统**，
   而文件好好的。现在按 `result.truncated` 分流：

   | 原因 | 判据 | 该去查什么 |
   | --- | --- | --- |
   | 预算装不下截断说明 | `truncated=True` 且正文空 | **预算（`max_tokens`）** |
   | 文件读不出来 / 只有 frontmatter | `truncated=False` 且正文空 | 权限 / 编码 / 文件内容 |

### 13.5 门槛 G79 与它的注入（`batch14` 第 5 条）

**测试**：`test_discard_stays_visible_at_every_body_budget` 参数化扫 9 档预算。

**注入**：`load_body` 去掉短标记退路（`markers=(marker,)`）→ 必须红。

**为什么档位下限取 25 而不是 15 —— 这条算式是本次最重要的产出之一**
...
代入 15 →  44 < 45 < 145   只差 1 个字符 ✗ —— 那是在碰运气
```

**这条判据不是新发明的**：§12.4 里 G75 的注入之所以选"倒序"而不是"删掉排序"，
用的是同一条思路——**门槛的注入必须"确定性会红"，否则报出来的
"注入后仍然全绿"是实验无效，不是门槛有效。**

### 13.6 实测：G79 在什么档位变红

跑 `gate_injection_batch14.py`，G79 红在 **`[60]` 档**——不是 25 档。
因为跑它的机器上 `tmp_path` 比系统 Temp 更长（前缀 `pytest-of-刘康鑫/pytest-N/test_...0`），
详细标记也就更长，60 档就已经装不下了。

**这恰好说明设计目标达成了**：长路径环境下 60 档红、短路径环境下至少 25 档红
——**两种环境都会红，只是红在不同的档位**。判据不依赖机器，档位依赖，这就对了。

### 13.7 本补丁的验收

**510 单测 / mypy 53 files clean / 契约 3 kept / 注入 78/78 / p2_compaction 2/2 / 回放 6/6 / 无残渣。**

（510 = 467 + 43，本批 `test_agent_skills.py` 由 31 增至 43 个用例。）

### 13.8 两条可复用的教训

1. **"单跑绿、全量红"不一定是顺序依赖，也可能是"环境让某条路径走不到"。**
   本例里 `tmp_path` 的长度就是一个隐藏输入。排查时先问：
   **这条测试的哪个输入是环境给的？**
2. **把"随环境变化的量"放进有预算上限的文案里，等于让同一段代码在不同机器上行为不同。**
   退路（短标记）不是冗余，它保证「丢弃可见」这条底线与环境无关。
